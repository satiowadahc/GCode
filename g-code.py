...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.Radio_Gcode_Skew = Radiobutton(self.master, text="Skew", value="skew",
                                            width="100", anchor=W)
        self.Radio_Gcode_Skew.configure(variable=self.gcode_op)
...
################################################################################
    ...
    def Write_Config_File(self, event):
        ...
        config.append('( by Scorch - 2013-2019 )')
        config.append("(=========================================================)")
        # BOOL
    ...
    def File_Read_Probe_data(self, filename):
        ...
        for line in fin:
            ...
            try:
                ...
                values_text = filter(None, values_text) #remove blank values
            except:
                values_text = ""
        ...
        if (side == "probe_n_cut") or (side == "probe_only") or (side =="probe_adjusted"):
            ...
            if (side == "probe_n_cut") or (side == "probe_adjusted"):
                gcode2 = self.g_rip.generategcode_probe(self.g_rip.probe_gcode,
                                                      ...
                                                      probe_offsetZ=float(self.probe_offsetZ.get()),
                                                      probe_safe =float(self.probe_safe.get()) ) #Added probe safe
    ...
    def Master_Configure(self, event, update=0, Plot_Flag=True):
        ...
        if abs(self.w-w)>10 or abs(self.h-h)>10 or update==1:
            ...
            Yloc = Yloc + 24
            self.Radio_Gcode_Skew.place(x=x_label_L, y=Yloc, width=150, height=23)
        ...
        if self.gcode_op.get() == "split":
            ...
            if (self.probe_data==[]):
                ...
                for line in self.probe_data:
                    ...
                    xp  = line[0] - Xoffset
                    yp  = line[1] - Yoffset
                    zp  = line[2] - Zoffset #addd "- Zoffset" V12
                    Lpt= xPartitionLength/5
                    self.coords.append([xp-Lpt, yp    , zp, xp+Lpt, yp    , zp, 2, 1 ])
                    self.coords.append([xp    , yp+Lpt, zp, xp    , yp-Lpt, zp, 2, 1 ])
                    self.coords.append([xp    , yp    ,  0, xp    , yp    , zp, 2, 1 ])
...
################################################################################  
    def STOCK_Round_Window(self):
        ...
        try: #Attempt to create temporary icon bitmap file
            ...
            f.write("   0x7f, 0xfe, 0x3f, 0xfc, 0x9f, 0xf9, 0xcf, 0xf3 };\n")
            f.close()
            stock_round_win.iconbitmap("@g_code_ripper_icon")
            os.remove("g_code_ripper_icon")
        except:
            pass
        ...
        self.GEN_Close = Button(stock_round_win,text="Close",command=self.Close_Current_Window_Click)
...
class G_Code_Rip:
            ...
            #for com in code_line:
                ...
                if com[0] == "G":
                    ...
                    if Gnum == "0" or Gnum == "1":
                        ...
                        for j in range(i+1,len(code_line)):
                            com = code_line[j]
                            passthru = passthru +  "%s%s " %(com[0],com[1])
                        break
                    else:
    ...
    def generate_probing_gcode(self,
        ...
        g_code.append("( by Scorch - 2013-2019 www.scorchworks.com                    )")
        if self.units == "in":
        ...
        #print max_probe_safe, probe_safe, probe_offsetZ
        g_code.append("G0 Z%.3f" %(max_probe_safe))
        xp = 0.0
        yp = 0.0
        for i in range(len(probe_coords)):
            if (probe_coords[i][0] or close_file):
                xp = probe_coords[i][2]
                yp = probe_coords[i][3]
        ...
        for line in side:
            ...
            if (line[0] == 1) or (line[0] == 2) or (line[0] == 3):
                ...
                if probe_data!=[]:
                    ...
                    #print line[2][0], line[2][1], line[2][2], line[2][3], line[2][4], line[2][5], line[2][6]
                    v102 = Z1 + F2*Z2 - F2*Z1
                    v101 = Z3 + F2*Z4 - F2*Z3
                    v100 = v102 + F1*v101 - F1*v102
                    Z_calculated = coordB[2] + v100 - probe_offsetZ #<------------------------------------------------3/26
                ...
                LINE = self.app_gcode_line(LINE,AXIS[0],coordB[0],DECP[0],WriteAll)
                LINE = self.app_gcode_line(LINE,AXIS[1],coordB[1],DECP[1],WriteAll)
                LINE = self.app_gcode_line(LINE,AXIS[2],coordB[2]+Z_probe_max,DECP[2],WriteAll)
                g_code.append(LINE)
